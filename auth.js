...
router.post('/register', (req, res) => {
    ...
    db.query(query, [username, email, hashedPassword], (err, result) => {
        if (err) {
            // Kirim kembali ke halaman register dengan pesan kesalahan
            return res.render('register', { error: 'Error registering user: ' + err.message });
        }
...
router.post('/login', (req, res) => {
    ...
    db.query(query, [username], (err, result) => {
        ...
        if (result.length > 0) { // Jika pengguna ditemukan
            ...
            console.log('User found in DB:', user); // Log user dari DB
